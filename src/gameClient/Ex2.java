...
/**
 * This class is the game client.
 * If you want to play the game you can do this through the command line or just click the jar file.
 */
public class Ex2 implements Runnable{
	...
	private static void moveAgants(game_service game, directed_weighted_graph gg) {		
		...
		for(int i=0;i<log.size();i++) {
			...
			if(dest==-1) {
				dest = nextNode(gg, ag);
				Arena.updateEdge(currFruit.get(ag.getID()),gg);
				if(currFruit.containsKey(ag.getID()))
				{
					if(currFruit.get(ag.getID()).get_edge()!= null && dest == currFruit.get(ag.getID()).get_edge().getDest())//Then the agent is one step from eating the agent
					{
						Pokemon tmp = currFruit.get(ag.getID());
						currFruit.remove(ag.getID());
